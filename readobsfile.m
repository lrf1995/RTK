...
% 读取观测数据
    if line(1)=='G'
        ...
        obsdata.epoch(epochnum).gps(gpsobs).gpst =gpst;% 用户接收机接收机信号接收时间tu
        obsdata.epoch(epochnum).gps(gpsobs).prn = str2double(line(2:3));   % 用户接收的卫星prn号
        obsdata.epoch(epochnum).gps(gpsobs).CLC = str2double(line(5:17));  % 用户接收的卫星伪距量C1L
        obsdata.epoch(epochnum).gps(gpsobs).L1C = str2double(line(20:33)); % 用户接受的卫星载波相位L1C
