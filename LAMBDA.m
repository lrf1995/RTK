...
% solution.（这是LAMBDA软件包的主要程序。默认情况下，基于所提供的浮动模糊向量ahat和相关的方差-协方差矩阵Qahat，
%将使用盲降方法进行整数估计。然而，用户也可以选择其他方法:整数舍入、自举或部分模糊分辨率(PAR)。
%此外，还可以选择应用比率测试来决定是否接受固定的解决方案）。
...
% INPUTS:
%
%     ahat: Float ambiguities (must be a column!)       % 浮点模糊度解
%    Qahat: Variance/covariance matrix of ambiguities   % QN的协方差矩阵
%% 选用数据处理方式，多选用方式6
...
% varargin: Optional input arguments, see below    %可选输入参数    
%
% OUTPUTS:
%
...
%       mu: Threshold value used for Ratio Test           用于Ratio Test的门限值，取为1/3
%
%
