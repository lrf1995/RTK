...
% 计算卫星的坐标及接收机伪距
global flag lamda u OMEGAdote ; %地球自转角速度
a1=m;
satnum=0;
     ...
     if (isnan(P)||isnan(F)||(F==0)||(P==0)),continue;end      %判断数据的伪距和载波是否有数 
        for a3=1:length(navdata.system(flag).gps)           
         if(initdata.system(flag).epoch(a1).gps(a2).prn==navdata.system(flag).gps(a3).prn)
            tk = (initdata.system(flag).epoch(a1).gps(a2).gpst(1)-navdata.system(flag).gps(a3).gpst(1))*604800 + ...
                  initdata.system(flag).epoch(a1).gps(a2).gpst(2)-navdata.system(flag).gps(a3).toe - P/299792458;
              if (tk > 302400)
                  tk = tk - 604800;
              elseif (tk<-302400)
                  tk = tk + 604800;
...
%            end
       ...
       %  5.计算信号发射时刻的真近点角vk
        ...
        %   11.计算卫星在WGS-84地心地固直角坐标系（Xt,Yt,Zt）中的坐标（X,Y,Z）
            ...
            Z=yk1*sin(ik);
       % 从发射时间转换到接收时间坐标系
        dw = OMEGAdote*(initdata.system(flag).epoch(a1).gps(a2).C1C/299792458);%传播时间转过的角度
        cw = cos(dw);sw = sin(dw);
       ...
       %我的改变
           ...
           if theta>(5*pi/180)
            ...
            % 计算即在概略点处据卫星的距离与基站与卫星距离差、Ru、卫星角度theta
             ...
             satdata(satnum).FC = initdata.system(flag).epoch(a1).gps(a2).L1C*lamda-sbr;   
             satdata(satnum).pc = initdata.system(flag).epoch(a1).gps(a2).C1C-sbr;
           end
